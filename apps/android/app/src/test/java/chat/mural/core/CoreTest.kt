...
class CoreTest {
    ...
    @Test fun languageRegistryAndThemesStayStable() {
        assertEquals(listOf("nb","es","en","fr","de","it","pt","zh","ko"),LanguageRegistry.all.map { it.id })
        assertEquals(24,Themes.shared.map { it.id }.toSet().size)
        assertEquals("Salut !",LanguageRegistry.get("fr")!!.greeting)
        for ((id, locale, greeting) in listOf(Triple("de","de-DE","Hallo!"),Triple("it","it-IT","Ciao!"),Triple("pt","pt-BR","Olá!"),Triple("zh","zh-CN","你好！"),Triple("ko","ko-KR","안녕하세요!"))) {
